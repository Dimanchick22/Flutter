...
static void my_application_activate(GApplication* application) {
  ...
  gtk_window_set_title(window, "Planner+");
...
static void my_application_local_command_line(GApplication* application, gchar*** arguments, int* exit_status) {
  ...
  g_autoptr(GPtrArray) dart_args = g_ptr_array_new();

  for (int i = 1; i < argc; i++) {
    g_ptr_array_add(dart_args, argv[i]);
  }

  g_ptr_array_add(dart_args, nullptr);

  self->dart_entrypoint_arguments = g_strdupv(static_cast<gchar**>(dart_args->pdata));
...
MyApplication* my_application_new() {
  return MY_APPLICATION(g_object_new(my_application_get_type(),
                                     "application-id", "com.plannerplus.planner_plus",
